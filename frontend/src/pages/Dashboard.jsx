...
import { 
  TrendingUp, TrendingDown, Scale, Droplets, Flame,
  Heart, Moon, Activity, Zap, ArrowUpRight, ArrowDownRight
} from 'lucide-react'
    ...
    >
      ...
      <motion.div variants={item} className="mb-8">
        <h1 className="text-3xl font-bold mb-2">
          Moin, <span className="gradient-text">{profile.name}</span> 
        </h1>
        ...
        <div className="glass-card p-6">
          ...
          <div className="flex flex-wrap gap-3">
            {profile.goals.map((goal, index) => (
              <span 
                key={index}
                className="px-4 py-2 rounded-full bg-gradient-to-r from-cyan-500/20 to-purple-500/20 border border-cyan-500/30 text-sm font-medium"
              >
                {goal}
              </span>
            ))}
      ...
      <motion.div variants={item} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {vitals.map((vital, index) => (
          <motion.div
            ...
            className={`stat-card ${vital.shadow}`}
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <div className={`flex items-center gap-1 text-sm ${
                vital.trend === 'down' ? 'text-green-400' : 'text-cyan-400'
              }`}>
                {vital.trend === 'down' ? (
            ...
            <p className="text-3xl font-bold">
              {vital.value}
              <span className="text-lg text-gray-400 ml-1">{vital.unit}</span>
            </p>
          </motion.div>
        ))}
      </motion.div>
